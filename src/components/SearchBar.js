    ...
    <div className="relative w-full">
      <div className="relative">
        <input
          ...
          className="w-full px-3 py-2 md:px-4 md:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm md:text-sm"
        />
              ...
              >
                <div className="flex items-center">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{internship.students.length} student{internship.students.length !== 1 ? 'en' : ''}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {searchTerm && filteredInternships.length === 0 && (
        <div className="absolute top-full left-0 right-0 mt-1 p-3 text-center text-gray-500 text-sm bg-white rounded-lg shadow-lg border border-gray-200 z-50">
          Geen bedrijven gevonden voor "{searchTerm}"
        </div>
      )}
    </div>
  );
